...
public class SaEval extends SaDepthFirstVisitor <Integer> {
	...
	public Integer visit(SaVarSimple node)
	{
		...
		/** PATCH : parfois des tsItem sont null **/
		if(node.tsItem == null){
			//System.out.println(node.getNom());
			node.tsItem = this.tableGlobale.getVar(node.getNom());

			for(TsItemFct tsItemFct : this.tableGlobale.fonctions.values()){
				if(tsItemFct.getTable().getVar(node.getNom()) != null){
					node.tsItem = tsItemFct.getTable().getVar(node.getNom());
				}
			}
		}
		/******************************************/
